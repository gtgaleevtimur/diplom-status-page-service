...
func GetResultSMSData(path string, wg *sync.WaitGroup) [][]entities.SMSData {
	out := make(chan [][]entities.SMSData)
	wg.Add(1)
	go func() {
		defer wg.Done()
		defer close(out)
		first := smsReader(path)
		second := []entities.SMSData{}
...
//Функция создания хэш-таблицы с кодами alpha-2
...
//Функция сбоа данных о системе смс
...
//Функция проверки валидности данных смс
...
//Функция замены кода альфа-2 на настоящее название страны
